  ...
  return (
    <div style={{ maxWidth: 680, margin: '0 auto' }}>
      <div style={card}>
          ...
          <div style={{ background: bgSubtle, border: `1px solid ${borderSub}`, borderRadius: 10, padding: '12px 14px', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {keywords.map(keyword => (
                <span key={keyword} style={{ fontSize: 11, padding: '3px 9px', borderRadius: 99, background: D ? '#172554' : '#DBEAFE', color: D ? '#93c5fd' : '#1E3A8A', fontWeight: 500 }}>
                  {keyword}
                </span>
              ))}
            </div>
        ...
        {/* Button */}
        <button onClick={tailor} disabled={loading || !resume || !jobDesc.trim()} style={{ width: '100%', padding: '11px 0', borderRadius: 12, border: 'none', background: loading ? '#1E3A8A' : '#1D4ED8', color: '#fff', fontSize: 13, fontWeight: 500, cursor: loading || !resume || !jobDesc.trim() ? 'not-allowed' : 'pointer', opacity: !resume || !jobDesc.trim() ? 0.5 : 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontFamily: 'inherit', transition: 'background 0.15s' }}>
      ...
      {!resume && (
        <div style={{ background: bg, border: `1px solid ${borderCol}`, borderRadius: 16, padding: '40px 32px', textAlign: 'center' }}>
          <div style={{ width: 56, height: 56, borderRadius: 14, background: 'rgba(29,78,216,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: 24 }}>🎯</div>
