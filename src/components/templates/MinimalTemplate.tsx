    ...
    <div id="resume-output" style={{
      ...
      {resume.languages?.length > 0 && (
        <MinSection title="Languages" border={borderCol}>
          <p style={{ fontSize: 13, color: textMuted, margin: 0, lineHeight: 1.7 }}>
            {resume.languages.map(lang => typeof lang === 'string' ? lang : `${lang.name}${lang.level ? ` (${lang.level})` : ''}`).join(', ')}
          </p>
        </MinSection>
      )}
        ...
        <MinSection title="Achievements" border={borderCol}>
          <ul style={{ paddingLeft: '1.2em', margin: 0 }}>
            {resume.achievements.map((a, i) => (
              <li key={i} style={{ fontSize: 13, marginBottom: 6, color: textMuted, lineHeight: 1.55 }}>
                <strong style={{ color: textC }}>{a.title}</strong>
                {(a.organization || a.date) && (
                  <span> — {[a.organization, a.date].filter(Boolean).join(' · ')}</span>
                )}
                {a.description && (
                  <div style={{ marginTop: 2 }}>{a.description}</div>
                )}
              </li>
            ))}
          </ul>
