    ...
    <div id="resume-output" style={{
      ...
      <aside style={{ background: sidebar, color: '#fff', padding: roomy ? '17mm 9.5mm 14mm' : '13mm 8.5mm 10mm' }}>
        ...
        <SidebarSection title="Contact" density={density}>
          ...
          {resume.portfolio && <SidebarLine>{resume.portfolio}</SidebarLine>}
          {resume.github && <SidebarLine>{resume.github}</SidebarLine>}
        </SidebarSection>
          ...
          <MainSection title="Achievements" density={density}>
            <ul style={{ margin: 0, paddingLeft: 15 }}>
              {resume.achievements.map((achievement, i) => (
                <li key={i} style={{ marginBottom: gap(4, 3, 7), color: bodyText, fontSize: px(11.3, 10.8, 13.6), lineHeight: roomy ? 1.6 : 1.45 }}>
                  <strong>{achievement.title}</strong>
                  {(achievement.organization || achievement.date) && (
                    <span> — {[achievement.organization, achievement.date].filter(Boolean).join(' · ')}</span>
                  )}
                  {achievement.description && (
                    <div style={{ marginTop: 2 }}>{achievement.description}</div>
                  )}
                </li>
              ))}
            </ul>
