...
export function downloadProjectBackup(project: ResumeProjectBackup) {
  ...
  const safeName =
    (project.resume?.name || project.form.name || 'resume')
      .replace(/[^\w\s-]/g, '')
      .trim()
...
function buildHTMLString(r: ResumeOutput): string {
  ...
  const languages = (r.languages || [])
    .map((language) => {
      ...
      return `<span class="tag neutral">${esc(text)}</span>`;
    })
    .join('');
  ...
  const achievements = (r.achievements || [])
    .map((a) => {
      ...
      const details = [a.organization, a.date].filter(Boolean).map(esc).join(' · ');
  ...
  <div class="header-row">
    ...
    <div class="identity">
      ...
      <div class="contacts">
        ...
        ${r.portfolio ? `<a href="${esc(r.portfolio)}">${esc(r.portfolio)}</a>` : ''}
        ${r.github ? `<a href="${esc(r.github)}">${esc(r.github)}</a>` : ''}
      </div>
    </div>
  </div>
...
export function copyAsText(r: ResumeOutput): string {
  ...
  const achievements = (r.achievements || []).map((a) => {
    const details = [a.organization, a.date].filter(Boolean).join(' · ');
    return `• ${a.title}${details ? ` — ${details}` : ''}${a.description ? ` — ${a.description}` : ''}`;
  });

  return [
    r.name,
    r.title,
    [r.email, r.phone, r.location, r.linkedin, r.portfolio, r.github]
      .filter(Boolean)
      .join(' | '),
    '',
    ...
    'EDUCATION',
    ...(r.education || []).map(
      (e) => `${e.institution} — ${e.degree} | ${e.period}`
    ),
    '',
    'SKILLS',
    (r.skills || []).join(', '),
    ...(r.languages?.length ? ['', 'LANGUAGES', languages] : []),
