          ...
          <div style={{ marginTop: 20 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {resume.languages.map((s, i) => (
                <span key={i} style={{ fontSize: 10.8, color: 'rgba(255,255,255,0.85)' }}>{s}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div style={{ padding: '14mm 13mm', color: textC }}>
        ...
        {resume.achievements?.length > 0 && (
          <div>
            ...
            <ul style={{ paddingLeft: '1.2em', margin: 0 }}>{resume.achievements.map((a, i) => <li key={i} style={{ fontSize: 12.2, marginBottom: 3, color: textMuted, lineHeight: 1.5 }}>{a}</li>)}</ul>
          </div>
        )}
      </div>
    </div>
